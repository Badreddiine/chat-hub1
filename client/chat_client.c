 ...
 * TCP  : auth, privés, commandes, keepalive, réception MSG_PUBLIC fallback
 * UDP  : réception broadcasts salon général (port aléatoire)
 */
...
static void *recv_thread(void *arg) {
    ...
    while (running) {
        ...
        if (FD_ISSET(tcp_fd, &fds)) {
            ...
            if (n <= 0) {
                printf("\n[INFO] Connexion perdue avec le serveur.\n");
                running = 0; break;
            }
            switch (m.type) {
            case MSG_CONNECT_OK:
                print_system(m.body); break;
            case MSG_CONNECT_ERR:
                print_system(m.body); running = 0; break;
            case MSG_SYSTEM:
                print_system(m.body); break;
            case MSG_PUBLIC:
                /* fallback TCP pour messages publics */
                if (strcmp(m.from, my_pseudo) != 0)
                    print_public(m.from, m.body);
                break;
            case MSG_PRIVATE:
                print_private(m.from, m.body); break;
            case MSG_PING: {
...
static int connect_to_server(const char *host, int tcp_port) {
    ...
    /* UDP — port aléatoire pour éviter les conflits */
    ...
    /* Adresse UDP serveur (pour envoyer les messages publics) */
...
int main(int argc, char *argv[]) {
    ...
    if (!running) {
        ...
        close(tcp_fd); close(udp_fd); exit(1);
    }
